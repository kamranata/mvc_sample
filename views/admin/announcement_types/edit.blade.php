...
@section('title', __('Edit vacancy type'))

@section('content')

<form role="form" method="post" action="{{ route('admin.vacancy_types.update', $vacancy_type) }}" autocomplete="off">
    @csrf
    @method('PUT')
        ...
        <div class="col-span-12 lg:col-span-6 box">
            ...
            <div class="post__content tab-content">
                ...
                <div id="locale-{{ $code }}" class="tab-pane p-5 {{ ($code == $default_locale ? 'active' : '') }}" role="tabpanel" aria-labelledby="locale-{{ $code }}-tab">
                    <div class="mb-3">
                        ...
                        <input type="text" name="{{ $code }}[title]" value="{{ old($code . '.title', $vacancy_type->translate($code)->title ?? '') }}" class="form-control">
                    </div>
                </div>
                @endforeach
            </div>
            ...
            <div class="p-5 border-t border-gray-200 grid grid-cols-2 gap-5 items-end">
                <div>
                    ...
                    <input type="text" name="ordering" value="{{ old('ordering', $vacancy_type->ordering) }}" class="form-control">
                </div>
                <div class="form-check">
                    <input id="changeable" name="changeable" value="1" class="form-check-switch" type="checkbox" {{ old('changeable', $vacancy_type->changeable) ? 'checked' : '' }}>
                ...
                <table class="table table--sm">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <input id="country-{{ $country->id }}" type="text" name="amount[{{ $country->id }}]" value="{{ old('amount.'.$country->id, $values[$country->id] ?? 0) }}" class="form-control">
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
    ...
    <div class="box p-5 mt-5">
        <div class="flex justify-between">
            <div class="form-check">
                <input id="status" name="status" value="1" class="form-check-switch" type="checkbox" {{ old('status', $vacancy_type->status) ? 'checked' : '' }}>
